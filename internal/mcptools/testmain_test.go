...
// package runs. RegisterTools reaches the package-level config.Get() cache,
// which now requires a non-empty OPENBRAIN_DB_PASSWORD and panics otherwise.
// Each Go test binary is its own process, so this env var must be set here
// independently of any other package's TestMain. A value already set in the
